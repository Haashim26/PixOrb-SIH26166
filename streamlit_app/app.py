...
This is a thin UI wrapper around the same `pixorb` registration pipeline
...
def text_value(value):
    """Format general values for display."""
    if value is None:
        return "—"
...
if run_clicked:
    ...
    with st.spinner(
        ...
        # Preserve the uploaded file extensions.
        ref_suffix = Path(reference_file.name).suffix.lower()
        src_suffix = Path(source_file.name).suffix.lower()
        ...
        try:
            ...
            st.session_state.pop("last_results_dir", None)
...
if "last_result" in st.session_state:
    ...
    results_dir = Path(st.session_state["last_results_dir"])
    ...
    with quality_cols[0]:
        metric_card(
            ...
            "Root mean square geometric residual of the final verified matches.",
        )
    ...
    with quality_cols[1]:
        metric_card(
            ...
            "Mean geometric reprojection error after the final transform.",
        )
    ...
    with quality_cols[2]:
        metric_card(
            ...
            "Median geometric reprojection error of final matches.",
        )
    ...
    with quality_cols[3]:
        metric_card(
            ...
            "Largest geometric reprojection residual among final matches.",
        )
    ...
    with matching_cols[0]:
        metric_card(
            "Candidate matches",
            integer(metrics.get("candidate_matches")),
            "Learned feature correspondences before final geometric filtering.",
        )

    with matching_cols[1]:
        metric_card(
            "Final matches",
            integer(metrics.get("final_match_count")),
            "Correspondences retained after the final verification stage.",
        )

    with matching_cols[2]:
        metric_card(
            "RANSAC inlier ratio",
            percentage(metrics.get("final_ransac_inlier_ratio")),
            "Final geometrically consistent matches divided by final matches.",
        )

    with matching_cols[3]:
        metric_card(
            "Overall retained",
            percentage(metrics.get("overall_retained_ratio")),
            "Final retained matches relative to candidate matches.",
        )


    # =======================================================================
    # 3. Initial / final verification
    # =======================================================================
    ...
    with ransac_cols[0]:
        metric_card(
            ...
            integer(metrics.get("initial_ransac_inlier_count")),
        )

    with ransac_cols[1]:
        metric_card(
            "Initial inlier ratio",
            percentage(metrics.get("initial_ransac_inlier_ratio")),
        )

    with ransac_cols[2]:
        metric_card(
            "Final RANSAC inliers",
            integer(metrics.get("final_ransac_inlier_count")),
        )
    ...
    with spatial_cols[0]:
        metric_card(
            "Grid occupancy",
            percentage(metrics.get("grid_occupancy_ratio")),
            "Fraction of the spatial grid occupied by final matches.",
        )

    with spatial_cols[1]:
        metric_card(
            "Spatial entropy",
            number(metrics.get("spatial_entropy_normalized"), 3),
    ...
    with spatial_cols[2]:
        metric_card(
            "Occupied cells",
            integer(metrics.get("occupied_cells")),
        )

    with spatial_cols[3]:
        metric_card(
            "Total grid cells",
            integer(metrics.get("total_cells")),
        )
    ...
    with overlap_cols[1]:
        metric_card(
            "Overlap MAE",
            number(metrics.get("overlap_mae"), 3),
        )

    with overlap_cols[2]:
        metric_card(
            "Overlap RMSE",
            number(metrics.get("overlap_rmse"), 3),
        )

    with overlap_cols[3]:
        metric_card(
            "Sub-pixel updates",
            integer(metrics.get("subpixel_changed_count")),
        )
    ...
    with pipeline_cols[0]:
        metric_card(
            "Feature backend",
            text_value(metrics.get("backend")),
        )

    with pipeline_cols[1]:
        metric_card(
            "Geometric model",
            text_value(metrics.get("model")),
        )
    ...
    coarse = metrics.get("coarse_alignment", {})
    ...
    if isinstance(coarse, dict):
        ...
        with confidence_cols[0]:
            metric_card(
                "Scale confidence",
                number(coarse.get("scale_confidence"), 3),
            )

        with confidence_cols[1]:
            metric_card(
                "Translation confidence",
                number(coarse.get("translation_confidence"), 3),
            )

        with confidence_cols[2]:
            metric_card(
                "Pyramid levels",
                integer(coarse.get("pyramid_levels")),
            )

        with confidence_cols[3]:
            metric_card(
                "Estimation level",
                integer(coarse.get("estimation_level")),
            )
    ...
    for row_start in range(0, len(image_labels), 3):

        row_items = image_labels[row_start:row_start + 3]

        img_cols = st.columns(3)

        for col, (fname, label) in zip(img_cols, row_items):
    ...
    downloads = [
        ("registered.png", "⬇️ Registered image"),
        ("match_points.csv", "⬇️ Match points — CSV"),
        ("match_points.json", "⬇️ Match points — JSON"),
        ("metrics.json", "⬇️ Metrics — JSON"),
    ]


    download_cols = st.columns(4)

    for col, (fname, label) in zip(download_cols, downloads):
        ...
        with col:
            ...
            if fpath.exists():
                ...
                st.caption(f"{fname} not available.")
    ...
    with st.expander("PixOrb processing pipeline"):
